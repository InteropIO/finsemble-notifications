...
const listOfWebpackEntryFiles = [path.join(__dirname, "./finsemble.webpack.json")];

let componentsToBuild = {};
listOfWebpackEntryFiles.forEach(function(filename) {
	const entries = fs.existsSync(filename) ? require(filename) : {};
...
for (const key in componentsToBuild) {
	const component = componentsToBuild[key];
...
module.exports = {
	...
	plugins: [
		new CopyWebpackPlugin([
			{ from: "components/**/*.html", to: "./", flatten: false },
			{ from: "components/**/*.jpg", to: "./", flatten: false },
			{ from: "components/**/*.css", to: "./", flatten: false },
			{ from: "components/**/*.json", to: "./", flatten: false },
			{ from: "components/**/*.png", to: "./", flatten: false },
